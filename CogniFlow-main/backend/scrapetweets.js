  ...
  return new Promise((resolve, reject) => {
    ...
    stream.on('data', (row) => {
      try {
        ...
        // Calculate engagement: Likes + Replies + Retweets (standard Twitter engagement metric)
        const likes = parseInt(row['Like Count'] || 0);
        const replies = parseInt(row['Reply Count'] || 0);
        const retweets = parseInt(row['Retweet Count'] || row['Repost Count'] || 0);
